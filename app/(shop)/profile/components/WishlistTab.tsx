      ...
      <header className="rounded-2xl border border-brand-border bg-brand-white p-4 shadow-sm sm:rounded-3xl sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-brand-red/10 text-brand-red">
            ...
            <AnimatePresence initial={false} mode="popLayout">
              {previewItems.map((item) => (
                <motion.li
                  ...
                  className="group flex gap-3 overflow-hidden rounded-2xl border border-brand-border bg-brand-white p-3 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-red hover:shadow-md"
                >
                <Link
                  href={`/products/${item.id}`}
                  className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-brand-light-bg min-[380px]:h-20 min-[380px]:w-20"
                >
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  ...
                  <Link
                    href={`/products/${item.id}`}
                    className="mt-0.5 line-clamp-2 text-sm font-bold text-brand-black hover:text-brand-red"
                  >
                    {item.name}
                  </Link>
